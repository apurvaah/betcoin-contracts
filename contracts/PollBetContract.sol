...
//    }
    ...
    function closePoll(uint _pollId) public payable returns(uint[] memory, uint[] memory) {
        Poll storage poll = polls[_pollId];
        ...
        (uint[] memory winningBets, uint[] memory winningBetWinnings) = distributeRewards(_pollId, winningPoll);

        return (winningBets, winningBetWinnings);
    } 

    // THIS SHOULD BE PRIVATE INSTEAD OF PUBLIC
    function distributeRewards(uint _pollId, uint _winningPollChoice) public payable returns(uint[] memory, uint[] memory){
        Bet[] storage bets = pollBetsList[_pollId];
        ...
        uint[] memory winningBetWinnings = calculateWinnings(_pollId, _winningPollChoice, winningBets);
        ...
        return (winningBets, winningBetWinnings); // currently just returns the indexes of winning bets - need to update
        // NEED TO EMIT UPDATED EVENT WITH INFO ON DISTRIBUTED REWARDS

    }

    function calculateWinnings(uint _pollId, uint _winningPollChoice, uint[] memory _winningBets) public view returns(uint[] memory){
        Bet[] storage bets = pollBetsList[_pollId];
        ...
        for (uint i = 0; i < winningBetAmounts.length; i++){
            winningBetWinnings[winningBetWinningsIdx] = ((winningBetAmounts[i]*100 / totalWinningBetAmounts)*totalBetAmount)/100;
            winningBetWinningsIdx += 1;
        }

        return winningBetWinnings;
    }
